...
export default function Header() {
  ...
  // Close quick toggle when clicking outside
  useEffect(() => {
    if (!showQuickToggle) return;

    const handleClickOutside = (event) => {
      const panel = document.getElementById('quick-toggle-panel');
      const button = event.target.closest('[data-toggle-button]');
      
      if (panel && !panel.contains(event.target) && !button) {
        setShowQuickToggle(false);
      }
    };
  ...
  const loadSystemConfig = async () => {
    try {
      ...
      setSystemConfig({
        ...
        enableFeedback: data.enableFeedback !== undefined ? data.enableFeedback : true
      });
    } catch (error) {
  ...
  const toggleFeature = async (feature) => {
    try {
      ...
      if (response.ok) {
        ...
        const featureName = feature === 'enableReport' ? 'Báo cáo' : 'Phản hồi';
        alert(`✅ Đã ${newValue ? 'BẬT' : 'TẮT'} chức năng ${featureName} cho tất cả users!`);
      } else {
  ...
  return (
    <>
        ...
        >
          ...
          {isAdmin ? (
            <>
              ...
              {/* Quick Toggle Button for Report & Feedback */}
              <button
                data-toggle-button
                onClick={() => setShowQuickToggle(!showQuickToggle)}
                title="Bật/Tắt Báo cáo & Phản hồi"
                style={{
                  background: "none",
                  border: "none",
                  cursor: "pointer",
      ...
      {/* ⚙️ Quick Toggle Panel for Report & Feedback */}
      {isAdmin && showQuickToggle && (
        <div
          id="quick-toggle-panel"
          style={{
            position: "fixed",
            top: 70,
            right: 20,
        ...
        >
          ...
          <div style={{
            ...
            <h3 style={{ margin: 0, color: "#b84e00", fontSize: "16px" }}>
              ⚙️ Quản lý tính năng
            </h3>
            <button
              onClick={() => setShowQuickToggle(false)}
              style={{
                background: "none",
                border: "none",
                cursor: "pointer",
          ...
          <div style={{ fontSize: "12px", color: "#6c757d", marginBottom: "15px", fontStyle: "italic" }}>
            ⚠️ Áp dụng cho <strong>TẤT CẢ USERS</strong>
          </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: "bold", color: "#495057", fontSize: "14px", marginBottom: "4px" }}>
                  📝 Chức năng Báo cáo
                </div>
                <div style={{ fontSize: "11px", color: "#6c757d" }}>
                  User có thể gửi báo cáo
                </div>
              </div>
              <button
                onClick={() => toggleFeature('enableReport')}
                style={{
                  padding: "6px 12px",
                  background: systemConfig.enableReport 
                  ...
                  minWidth: "65px",
                  transition: "all 0.3s ease",
                  boxShadow: systemConfig.enableReport 
                    ? "0 2px 8px rgba(40, 167, 69, 0.3)" 
                    : "0 2px 8px rgba(220, 53, 69, 0.3)"
                }}
                onMouseOver={(e) => {
                  e.target.style.transform = "scale(1.05)";
                  e.target.style.boxShadow = systemConfig.enableReport 
              ...
              >
                {systemConfig.enableReport ? "✅ BẬT" : "❌ TẮT"}
              </button>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: "11px", color: "#6c757d" }}>
                  User có thể xem trạng thái
                </div>
              </div>
              <button
                onClick={() => toggleFeature('enableFeedback')}
                style={{
                  padding: "6px 12px",
                  background: systemConfig.enableFeedback 
                    ? "linear-gradient(135deg, #28a745 0%, #20c997 100%)" 
                    : "linear-gradient(135deg, #dc3545 0%, #c82333 100%)",
                  color: "white",
                  border: "none",
                  ...
                  minWidth: "65px",
                  transition: "all 0.3s ease",
                  boxShadow: systemConfig.enableFeedback 
                    ? "0 2px 8px rgba(40, 167, 69, 0.3)" 
                    : "0 2px 8px rgba(220, 53, 69, 0.3)"
                }}
                onMouseOver={(e) => {
                  e.target.style.transform = "scale(1.05)";
                  e.target.style.boxShadow = systemConfig.enableFeedback 
                    ? "0 4px 12px rgba(40, 167, 69, 0.4)" 
                    : "0 4px 12px rgba(220, 53, 69, 0.4)";
                }}
                onMouseOut={(e) => {
              ...
              >
                {systemConfig.enableFeedback ? "✅ BẬT" : "❌ TẮT"}
              </button>
            </div>
          </div>
          ...
          <div style={{
            ...
            💡 <strong>Lưu ý:</strong> Thay đổi sẽ áp dụng cho tất cả users sau ~10 giây
          </div>
        </div>
      )}
    </>
